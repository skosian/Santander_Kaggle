...
# Group factos with many levels, and create binary/dummy variabls for all factors

train.factors <- train[,c('ind_empleado','pais_residencia','sexo','ind_nuevo','indrel','indrel_1mes','tiprel_1mes',
                                                      'indresi','indext','conyuemp','canal_entrada','indfall','cod_prov','ind_actividad_cliente','segmento')]

train.factors$canal_entrada_group=as.character(train.factors$canal_entrada)
train.factors$pais_residencia_group=as.character(train.factors$pais_residencia)

train.factors$canal_entrada_group[!(train.factors$canal_entrada_group %in% c('KHE','KAT','KFC','KHQ','KFA','KHK','','KHM','KHD','KHN','KAS','RED','KAG','KAY','KAA','KAB',
                                                               'KAE','KCC','KBZ','KHL','KFD','KAI','KEY','KAW','KAR','KAZ','KAF','7','13','KCI','KAH','KAJ',
                                                               'KCH','KHF','KAQ'))] <- "OTHER"

train.factors$pais_residencia_group[!(train.factors$pais_residencia_group %in% c("ES"))] <- "OTHER"


# train.factors %>% group_by(canal_entrada_group) %>% summarise(count=n())
# train.factors %>% group_by(pais_residencia_group) %>% summarise(count1=n())

# table(train.factors$canal_entrada_group,useNA = "ifany")
# table(train.factors$pais_residencia_group,useNA = "ifany")

# summary_by_canal_entrada <- filter(train100K_with_lags,!is.na(num_new_products)) %>% group_by(canal_entrada) %>% summarise(count=n(),avg=mean(num_new_products)) %>% arrange(desc(count))


# summary_by_canal_entrada_group <- filter(train100K_with_lags,!is.na(num_new_products)) %>% group_by(canal_entrada_group) %>% summarise(count=n(),avg=mean(num_new_products)) %>% arrange(desc(count))
# write.csv(summary_by_canal_entrada,file="C:/Santander/EDA/avg_new_products_by_canal_entrada.csv")

# drop the pre-group variables pais_residencia and canal_entrada
train.factors <- train.factors[,! (names(train.factors) %in% c("pais_residencia","canal_entrada"))]

train.factors$canal_entrada_group <- as.factor(train.factors$canal_entrada_group)
train.factors$pais_residencia_group <- as.factor(train.factors$pais_residencia_group)

dummies = model.matrix(~.+0,data = train.factors)
