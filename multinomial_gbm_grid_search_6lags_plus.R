...
train300K_with_lags$tipodom = train300K_with_lags$nomprov = train300K_with_lags$ult_fec_cli_1t = train300K_with_lags$fecha_alta = NULL

# Select only non-zer growth records, and create a column called new_product that identifies the new product name
for (i in 1:9) {
   assign(paste0("train_growth",i),subset(train300K_with_lags,get(top_products[i])==1))
...
mod_multinomial <- dplyr::filter(train_growth_all,fecha_dato >'2015-06-28' & fecha_dato <= '2016-03-28')
...
splits <- h2o.splitFrame(data = mod_multinomial.hex, ratios = c(0.7), destination_frames = c("train.hex", "valid.hex"), seed = 1234)

train_multinomial.hex <- splits[[1]]
valid_multinomial.hex <- splits[[2]]

x <- names(mod_multinomial)[!(names(mod_multinomial) %in% c(product_columns,product_columns_growth,exclude_columns,drop_list))]
...
hyper_params = list( max_depth = c(3,4,5), #6,7,8,10,12),
                     ...
                     #sample_rate_per_class=list(c(0.3,0.7,1,1,0.3,0.3,1,0.15,0.3),c(1,1,1,1,1,1,1,1,1))
                     )
...
sortedGrid_mulinomial <- h2o.getGrid(grid_id, sort_by = "accuracy", decreasing = TRUE)

print(sortedGrid_mulinomial)
