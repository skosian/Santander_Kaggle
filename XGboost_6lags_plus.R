...
mod=dplyr::filter(train100K_with_lags_dummies_imputed,fecha_dato >'2015-11-28' & fecha_dato <= '2016-03-28')
...
x <- names(mod)[!(names(mod) %in% c(product_columns,product_columns_growth,exclude_columns,biz_columns))]  #
...
for (i in c(11)) #length(top_products)) 
 {
  ...
  train_y_factor_sample <- as.factor(paste0("A",as.character(mod_train_sample[,top_products[i]])))
...
write.csv(get(paste0("Grid_results",i)),file=paste0("C:/Santander/Saved Grids/",model_date," XGboost/Grid_results_eta",eta,"_",i,".csv"))
