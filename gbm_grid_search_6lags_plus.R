...
train_hex_ids=data.frame(train_hex$ncodpers)

valid_hex = as.data.frame(valid.hex)
valid_hex_ids=data.frame(valid_hex$ncodpers)

test_hex = as.data.frame(test.hex)
test_hex_ids=data.frame(test_hex$ncodpers)

names(train_hex_ids) = names(valid_hex_ids) = names(test_hex_ids)=c("ncodpers")
...
assign(paste0("train_scores_20pct_",i),cbind(train_hex_ids,as.data.frame(get(paste0("fit.train_",i)))))
assign(paste0("valid_scores_20pct_",i),cbind(valid_hex_ids,as.data.frame(get(paste0("fit.valid_",i)))))
assign(paste0("test_scores_20pct_",i),cbind(test_hex_ids,as.data.frame(get(paste0("fit.test_",i))))) 
...
save(all_scores_combined_string,file="C:/Santander/Data/all_scores_combined_gbm_6lags_plus_scrubbed_Nov16_20pct_modifier.RData")


submission=all_scores_combined_string[,c("ncodpers","output_string")]
names(submission)=c("ncodpers","added_products")

write.csv(submission[,1:2],file="C:/Santander/Submissions/submission16_Nov16_gbm_6lags_plus_scrubbed_20pct_modifier.csv",row.names = FALSE)
