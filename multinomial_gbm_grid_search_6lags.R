...
for (i in 1:9) {
  # if (train100K_with_lags$top_products[i])
  assign(paste0("train_growth",i),subset(train100K_with_lags,get(top_products[i])==1))
  # assign(get(paste0("train_growth",i)$new_product),gsub("_growth","",top_products[i]))
...
exclude_columns=c("fecha_dato","ncodpers","fecha_alta","ult_fec_cli_1t","tipodom","nomprov","num_products","num_new_products",
                  "num_new_products_factor","new_product")
...
pred_products_train = h2o.predict(best_model_from_multinomial_grid,newdata = train_multinomial.hex)
pred_products_valid = h2o.predict(best_model_from_multinomial_grid,newdata = valid_multinomial.hex)
pred_products_test = h2o.predict(best_model_from_multinomial_grid,newdata = test_multinomial.hex)
pred_products_scoring = h2o.predict(best_model_from_multinomial_grid,newdata = scoring.hex)
...
write.csv(submission[,1:2],file="C:/Santander/Submissions/submission8_Nov10_multinomial_6lags.csv",row.names = FALSE)
