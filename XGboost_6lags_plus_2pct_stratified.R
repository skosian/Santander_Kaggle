...
# write.csv(stats.train100K_with_lags_dummies,file="C:/Santander/EDA/stats.train100K_with_lags_dummies.csv")
# 
...
# summary(test_with_lags_dummies_imputed$num_new_products_lag3)

# missing_columns <- c("age","antiguedad","renta","ind_nomina_ult1","ind_nom_pens_ult1")
# median.missing_columns <- stat.desc(train100K_with_lags_dummies[,missing_columns])[8,]
# 
# for (i in 1:length(missing_columns)) {
#   train100K_with_lags_dummies[,missing_columns[i]][is.na(train100K_with_lags_dummies[,missing_columns[i]])] <- median.missing_columns[1,i]
# }
...
mod=dplyr::filter(train100K_with_lags_dummies_imputed,fecha_dato >'2015-06-28' & fecha_dato <= '2016-03-28')
val=dplyr::filter(train100K_with_lags_dummies_imputed,fecha_dato > '2016-03-28')
...
mod_train=dplyr::sample_frac(mod,size=0.7)
mod_valid=dplyr::anti_join(mod,mod_train[,c("ncodpers","fecha_dato")])


x <- names(mod)[!(names(mod) %in% c(product_columns,product_columns_growth,exclude_columns))]
...
names(test_raw_ids)=c("ncodpers")
  ...
  mod_train_short <- mod_train_short[,order(colnames(mod_train_short))]
  mod_valid_short <- mod_valid_short[,order(colnames(mod_valid_short))]
  test_short <- test_short[,order(colnames(test_short))]
  scoring_short <- scoring_short[,order(colnames(scoring_short))]
...
for (i in 1:12) #length(top_products)) 
 {
  ...
  ## stratified sample where 0's are 19 times more than 1's, i.e. event rate is 5%
  mod_train_ones=mod_train[mod_train[,top_products[i]]==1,]
  mod_train_zeros=mod_train[mod_train[,top_products[i]]==0,]
  mod_train_stratified = rbind(mod_train_ones,dplyr::sample_n(mod_train_zeros,49*nrow(mod_train_ones)))  
  mean(as.numeric(as.character(mod_train_stratified[,top_products[i]])))
  
  mod_train_short <- mod_train_stratified[,x]
  mod_train_short <- mod_train_short[,order(colnames(mod_train_short))]
  # end stratified sampling
  
  # train_y <- as.numeric(as.character(mod_train[,top_products[i]]))
  train_y <- as.numeric(as.character(mod_train_stratified[,top_products[i]]))
  ...
  train_y_factor <- as.factor(paste0("A",as.character(mod_train_stratified[,top_products[i]])))
  ...
  # names(train_y) <-names(valid_y) <- top_products[i]
...
write.csv(get(paste0("Grid_results",i)),file=paste0("C:/Santander/Saved Grids/Nov28_XGboost/Grid_results_2perc_eta0.005_",i,".csv"))

# Retrieve previously saved grid search results
# assign(paste0("Grid_results",i),read.csv(paste0("C:/Santander/Saved Grids/Nov28_XGboost/Grid_results_2perc_",i,".csv")))
...
xgb.save(xgbFull, paste0("C:/Santander/Saved Models/Nov28 XGboost/xgbFull_2perc_eta0.005_",i))

# Load a previously saved model
# xgbFull <- xgb.load(paste0("C:/Santander/Saved Models/Nov28 XGboost/xgbFull_",i))
...
perf_metric <- as.data.frame(cbind(auc_train,auc_valid,auc_test,get(paste0("best_iter_",i))))
names(perf_metric) <- c("auc_train","auc_valid","auc_test","best_iteration")

assign(paste0("best_params",i),cbind(perf_metric,best_params,get(paste0("mean_predictions_temp_",i))))

# Create scoring datasets
assign(paste0("train_scores_",i),cbind(mod_train_stratified[,c("ncodpers","fecha_dato")],predicted_train,train_y))
...
eq <-  paste0("save(train_scores_",i,", valid_scores_",i,", test_scores_",i,", scores_",i,",file='C:/Santander/Saved Models/Nov28 XGboost/Scored Data/model_2perc_eta0.005_",i,"_scores.RData')")
eval(parse(text=eq))
...
Calibration(
  ...
  outputDir = "C:/Santander/Saved Models/Nov28 XGboost/Calibration/")
...
best_models_Nov28_2perc_eta0.005 <- best_models
write.csv(best_models_Nov28_2perc_eta0.005,file="C:/Santander/Stats/best_models_Nov28_2perc_eta0.005.csv")
...
write.csv(submission[,1:2],file="C:/Santander/Submissions/submission30_XGboost_6lags_plus_12prod_isotonic_calibration_Nov24.csv",row.names = FALSE)
