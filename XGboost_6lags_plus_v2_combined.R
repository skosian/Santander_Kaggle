...
train100K_with_lags_dummies$tipodom = train100K_with_lags_dummies$nomprov = train100K_with_lags_dummies$ult_fec_cli_1t = train100K_with_lags_dummies$fecha_alta = NULL

####################################################################################################################################
### MISSING VALUE IMPUTATION: Impute missing values with medians ###################################################################
####################################################################################################################################
all_columns <- names(train100K_with_lags_dummies)
stats.train100K_with_lags_dummies <- stat.desc(train100K_with_lags_dummies[,all_columns])

write.csv(stats.train100K_with_lags_dummies,file="C:/Santander/EDA/stats.train100K_with_lags_dummies_Nov30.csv")
load("C:/Santander/EDA/stats.train100K_with_lags_dummies_Nov30.csv")
# 
# stats.test_with_lags_dummies <- stat.desc(test_with_lags_dummies)
# write.csv(stats.test_with_lags_dummies,file="C:/Santander/EDA/stats.test_with_lags_dummies.csv")

numeric_columns_train100K <- names(train100K_with_lags_dummies[,sapply(train100K_with_lags_dummies,is.numeric)])
numeric_columns_test <- names(test_with_lags_dummies[,sapply(test_with_lags_dummies,is.numeric)])
...
names(train100K_with_lags_dummies)[order(names(train100K_with_lags_dummies))]


train100K_with_lags_dummies_imputed <- train100K_with_lags_dummies

# imputation with median
for (i in 1:length(numeric_columns_train100K)) {
  train100K_with_lags_dummies_imputed[,numeric_columns_train100K[i]][is.na(train100K_with_lags_dummies_imputed[,numeric_columns_train100K[i]])] <- stats.train100K_with_lags_dummies[8,numeric_columns_train100K[i]]  
}

test_with_lags_dummies_imputed <- test_with_lags_dummies

for (i in 1:length(numeric_columns_test)) {
  test_with_lags_dummies_imputed[,numeric_columns_test[i]][is.na(test_with_lags_dummies_imputed[,numeric_columns_test[i]])] <- stats.train100K_with_lags_dummies[8,numeric_columns_test[i]]  
}

##############################################################################################################################################

exclude_columns=c("fecha_dato","ncodpers","fecha_alta","ult_fec_cli_1t","tipodom","nomprov","num_products","num_new_products","num_new_products_factor","delta_products")

# Set the response variables as factor
for (i in 1:length(top_products)) {
train100K_with_lags_dummies_imputed[,top_products[i]] = as.factor(train100K_with_lags_dummies_imputed[,top_products[i]])
}

train100K_with_lags_dummies_imputed$num_new_products_factor=as.factor(train100K_with_lags_dummies_imputed$num_new_products)

## Drop any records where target variable is missing (XGboost does not like it)

for (i in 1:length(top_products)) {
  eq <-  paste0("train100K_with_lags_dummies_imputed <- dplyr::filter(train100K_with_lags_dummies_imputed,!is.na(",top_products[i],"))")
  eval(parse(text=eq))
}


save(test_with_lags_dummies_imputed,file="C:/Santander/Data/test_with_lags_dummies_imputed.RData")
save(train100K_with_lags_dummies_imputed,file="C:/Santander/Data/train100K_with_lags_dummies_imputed.RData")

load("C:/Santander/Data/test_with_lags_dummies_imputed.RData")
load("C:/Santander/Data/train100K_with_lags_dummies_imputed.RData")

####################### Create combined response variables ############################################################

# Set the response variables as factor
for (i in 1:length(top_products)) {
  train100K_with_lags_dummies_imputed[,top_products[i]] = as.numeric(as.character(train100K_with_lags_dummies_imputed[,top_products[i]]))
}

mean(train100K_with_lags_dummies_imputed$ind_recibo_ult1_growth)

response_columns <- train100K_with_lags_dummies_imputed[,top_products]
top_combined_products <- paste0("combined_response",1:12)

combined_response_columns <- train100K_with_lags_dummies_imputed[,c("ncodpers","fecha_dato")]

for (i in 1:12) {
  
  if (i==1) {
    max_upto_product1 <- data.frame(combined_response1=response_columns[,top_products[1]])
  } else {
  max_eq <- paste0("max_upto_product",i," <- data.frame(combined_response",i," = apply(response_columns[,top_products[1:",i,"]],1,max))")
  # rename_eq=paste0("names(max_upto_product",i,") <- 'combined_response",i,"'")
  eval(parse(text = max_eq))
    }
  top_products[1:i]
  print(paste("Mean of combined response",i,"up to",top_products[i],"=",mean(get(paste0("max_upto_product",i))[,1])))
 combined_response_columns <- cbind(combined_response_columns,get(paste0("max_upto_product",i)))
}

combined_response_columns_factor <- as.data.frame(lapply(combined_response_columns[,top_combined_products],as.factor))

train100K_with_combined_resp <- cbind(train100K_with_lags_dummies_imputed,combined_response_columns_factor)

## Split into modeling and valiation sets by month

mod=dplyr::filter(train100K_with_combined_resp,fecha_dato >'2015-11-28' & fecha_dato <= '2016-03-28')
val=dplyr::filter(train100K_with_combined_resp,fecha_dato > '2016-03-28')

# mod_nonmissing <- na.omit(mod)
###############################################################################################################################################
####################### Some EDA ##############################################################################
numeric_columns <- names(mod[,sapply(mod,is.numeric)])

# cor_mod_all=stats::cor(mod[,numeric_columns],method=c("pearson"),use = "pairwise.complete.obs")
# write.csv(cor_mod_all,file = "C:/Santander/EDA/cor_mod_all.csv")
# 
# cor_mod_x=stats::cor(mod[,x],method=c("pearson"),use = "pairwise.complete.obs")
# write.csv(cor_mod_x,file = "C:/Santander/EDA/cor_mod_x.csv")
# 
# cor_mod_response_columns=stats::cor(train100K_with_lags_dummies[,top_products],method=c("pearson"),use = "pairwise.complete.obs")
# write.csv(cor_mod_response_columns,file = "C:/Santander/EDA/cor_response_columns.csv")
# 
# cor_response_to_all=stats::cor(train100K_with_lags_dummies[,c(top_products,numeric_columns)],method=c("pearson"),use = "pairwise.complete.obs")
# write.csv(cor_response_to_all,file = "C:/Santander/EDA/cor_response_to_all.csv")
# 
# write.csv(table(mod[,top_products]),file = "C:/Santander/EDA/response_column_cross_tabulation.csv")
################################################################################################################################################
...
x <- names(mod)[!(names(mod) %in% c(product_columns,product_columns_growth,exclude_columns,biz_columns,top_combined_products))]  #
...
names(test_raw_ids)=c("ncodpers")
  ...
  test_short <- val[,x]
  scoring_short <- test_with_lags_dummies_imputed[,x]
...
for (i in 1:12) #length(top_products)) 
 {
  print(paste("Starting with model",i,": Combined response up to",top_products[i]))
 
  train_y <- as.numeric(as.character(mod_train[,top_combined_products[i]]))
  # train_sample_y <- as.numeric(as.character(mod_train_sample[,top_combined_products[i]]))
  valid_y <- as.numeric(as.character(mod_valid[,top_combined_products[i]]))
  test_y <- as.numeric(as.character(val[,top_combined_products[i]]))
  
  train_y_factor <- as.factor(paste0("A",as.character(mod_train[,top_combined_products[i]])))
  # train_y_factor_sample <- as.factor(paste0("A",as.character(mod_train_sample[,top_combined_products[i]])))
  valid_y_factor <- as.factor(paste0("A",as.character(mod_valid[,top_combined_products[i]])))
  test_y_factor <- as.factor(paste0("A",as.character(val[,top_combined_products[i]])))
...
write.csv(importance,file=paste0("C:/Santander/Saved Models/Nov30 XGboost/Importance/variable_importance_model_",i,"_",model_date,"_eta",eta,".csv"))
...
mean_predictions_temp <- data.frame(i,top_combined_products[i],mean(predicted_train$pred),mean(predicted_valid$pred),mean(predicted_test$pred),mean(predicted_scoring$pred),
...
##########################################################################################################################
########## Create product level propensities from combined propensities by subtracting the previous number ###############
##########################################################################################################################
...
for (i in 1) {
  ...
  load(paste0("C:/Santander/Saved Models/",model_date," XGboost/Scored Data/model_eta",eta,"_",i,"_scores.RData"))
 
  prod_train_y <- as.numeric(as.character(mod_train[,top_products[i]]))
  prod_valid_y <- as.numeric(as.character(mod_valid[,top_products[i]]))
  prod_test_y <- as.numeric(as.character(val[,top_products[i]]))

  if (i==1) {
  prod_train_scores_1 <- train_scores_1
  prod_valid_scores_1 <- valid_scores_1
  prod_test_scores_1 <- test_scores_1
  prod_scores_1 <- scores_1
  
  prod_train_scores_1$pred.product <- prod_train_scores_1$pred
  prod_valid_scores_1$pred.product <- prod_valid_scores_1$pred
  prod_test_scores_1$pred.product <- prod_test_scores_1$pred
  prod_scores_1$pred.product <- prod_scores_1$pred
  
  prod_train_scores_1$prod_train_y <- prod_train_scores_1$train_y
  prod_valid_scores_1$prod_valid_y <- prod_valid_scores_1$valid_y
  prod_test_scores_1$prod_test_y <- prod_test_scores_1$test_y
  
  } else {
    
  eq <- paste0("prod_train_scores_",i," <- cbind(train_scores_",i,"[,-4],prod_train_y)")
  eval(parse(text = eq))
  eq <- paste0("prod_valid_scores_",i," <- cbind(valid_scores_",i,"[,-4],prod_valid_y)")
  eval(parse(text = eq))
  eq <- paste0("prod_test_scores_",i," <- cbind(test_scores_",i,"[,-4],prod_test_y)")
  eval(parse(text = eq))
  eq <- paste0("prod_scores_",i," <- scores_",i,"[,-4]")
  eval(parse(text = eq))
  
  
  # prod_valid_scores_2 <- cbind(valid_scores_2[,-4],prod_valid_y)
  # prod_test_scores_2 <- cbind(test_scores_2[,-4],prod_test_y)
  # prod_scores_2 <- scores_2
  # 
  eq <- paste0("prod_train_scores_",i,"$pred.product <- prod_train_scores_",i,"$pred - prod_train_scores_",i-1,"$pred")
  eval(parse(text = eq))
  eq <- paste0("prod_valid_scores_",i,"$pred.product <- prod_valid_scores_",i,"$pred - prod_valid_scores_",i-1,"$pred")
  eval(parse(text = eq))
  eq <- paste0("prod_test_scores_",i,"$pred.product <- prod_test_scores_",i,"$pred - prod_test_scores_",i-1,"$pred")
  eval(parse(text = eq))
  eq <- paste0("prod_scores_",i,"$pred.product <- prod_scores_",i,"$pred - prod_scores_",i-1,"$pred")
  eval(parse(text = eq))
  
  # Normalize product level scoress to the range of 0,1
    assign(paste0("prod_train_scores_",i),mutate(get(paste0("prod_train_scores_",i)),pred.product.norm = (pred.product-min(pred.product))/(max(pred.product)-min(pred.product))))
  assign(paste0("prod_valid_scores_",i),mutate(get(paste0("prod_valid_scores_",i)),pred.product.norm = (pred.product-min(pred.product))/(max(pred.product)-min(pred.product))))
  assign(paste0("prod_test_scores_",i),mutate(get(paste0("prod_test_scores_",i)),pred.product.norm = (pred.product-min(pred.product))/(max(pred.product)-min(pred.product))))
  assign(paste0("prod_scores_",i),mutate(get(paste0("prod_scores_",i)),pred.product.norm = (pred.product-min(pred.product))/(max(pred.product)-min(pred.product))))
  
  }
  auc_prod_train <- auc(prod_train_y, get(paste0("prod_train_scores_",i))$pred)[1]
  auc_prod_valid <- auc(prod_valid_y, get(paste0("prod_valid_scores_",i))$pred)[1]
  auc_prod_test <- auc(prod_test_y, get(paste0("prod_test_scores_",i))$pred)[1]


  ## Perform isotonic calibration on the product level validation scores
  pred.valid.ord <- dplyr::arrange(get(paste0("prod_valid_scores_",i)), pred.product)
  iRFit= stats::isoreg(pred.valid.ord$pred.product,pred.valid.ord[,"prod_valid_y"])
  
  eq_iso<- paste0("prod_train_scores_",i," <- cbind(prod_train_scores_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(prod_train_scores_",i,"$pred.product)))")
  eval(parse(text=eq_iso))
  eq_iso<- paste0("prod_valid_scores_",i," <- cbind(prod_valid_scores_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(prod_valid_scores_",i,"$pred.product)))")
  eval(parse(text=eq_iso))
  eq_iso<- paste0("prod_test_scores_",i," <- cbind(prod_test_scores_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(prod_test_scores_",i,"$pred.product)))")
  eval(parse(text=eq_iso))
  eq_iso<- paste0("prod_scores_",i," <- cbind(prod_scores_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(prod_scores_",i,"$pred.product)))")
  eval(parse(text=eq_iso))
  
  auc_prod_test.cal <- auc(get(paste0("prod_test_scores_",i))[,"prod_test_y"], get(paste0("prod_test_scores_",i))[,"pred.cal"])[1]

  # hist(prod_test_scores_3$pred.cal,100)
  # plot(prod_test_scores_3$pred.product.norm,prod_test_scores_3$pred.cal)
  
  print(paste("Model number:",i))
  print(paste("Product AUC training set: ",round(auc_prod_train,4)))
  print(paste("Product AUC validation set: ",round(auc_prod_valid,4)))
  print(paste("Product AUC test set: ",round(auc_prod_test,4)))
  print(paste("Isotonic Calibrated Product AUC test set: ",round(auc_prod_test.cal,4)))
  
  
  # Get the mean valus of predictions on all data sets
  mean_prod_model_predictions_temp <- data.frame(i,top_products[i],
  mean(get(paste0("prod_train_scores_",i))$pred.product),mean(get(paste0("prod_valid_scores_",i))$pred.product),mean(get(paste0("prod_test_scores_",i))$pred.product),mean(get(paste0("prod_scores_",i))$pred.product),
  mean(get(paste0("prod_train_scores_",i))$pred.cal),mean(get(paste0("prod_valid_scores_",i))$pred.cal),mean(get(paste0("prod_test_scores_",i))$pred.cal),mean(get(paste0("prod_scores_",i))$pred.cal),           
  mean(prod_train_y),mean(prod_valid_y),mean(prod_test_y))
  
  names(mean_prod_model_predictions_temp)=c("Model_ID","Product_Name","Mean_Train_Prediction","Mean_Valid_Prediction","Mean_Test_Prediction","Mean_Scoring_Prediction",
                                            "Mean_Calibrated_Train_Prediction","Mean_Calibrated_Valid_Prediction","Mean_Calibrated_Test_Prediction","Mean_Calibrated_Scoring_Prediction",
                                            "Mean_Train_Actual","Mean_Valid_Actual","Mean_Test_Actual")
  assign(paste0("mean_prod_model_predictions_temp_",i),mean_prod_model_predictions_temp)
  
  
  # Create a summary vector that has all model parameters for the best model and AUC values and mean predictions
  perf_metric_prod_model <- as.data.frame(cbind(auc_prod_train,auc_prod_valid,auc_prod_test,auc_prod_test.cal))
  names(perf_metric_prod_model) <- c("auc_prod_train","auc_prod_valid","auc_prod_test","auc_prod_test_isotonic")
  
  assign(paste0("best_prod_model_params",i),cbind(perf_metric_prod_model,get(paste0("mean_prod_model_predictions_temp_",i))))
  
  
  if (!exists("best_product_models")) best_product_models = data.frame()
    best_product_models <- dplyr::bind_rows(best_product_models,get(paste0("best_prod_model_params",i)))
  
    eq <-  paste0("save(prod_train_scores_",i,", prod_valid_scores_",i,", prod_test_scores_",i,", prod_scores_",i,",file='C:/Santander/Saved Models/",model_date," XGboost/Scored Data/prod_model_eta",eta,"_",i,"_scores.RData')")
    eval(parse(text=eq))
    
    write.csv(best_product_models,file=paste0("C:/Santander/Stats/best_product_models_calibrated_",model_date,"_eta",eta,".csv"))
}


############# Combine all scores and add product names to each #########################################

for (i in 1:12) {

  # load(paste0("C:/Santander/Saved Models/Nov28 XGboost/Scored Data/Best/model_2perc_best_",i,"_scores.RData"))
  load(paste0("C:/Santander/Saved Models/",model_date," XGboost/Scored Data/prod_model_eta",eta,"_",i,"_scores.RData"))
  
  if (i != 3) {
    assign(paste0("sel_prod_scores_",i),get(paste0("prod_scores_",i)))
  } else {
    assign(paste0("sel_prod_scores_",i),get(paste0("prod_scores_",i-1)))
  }
  
    eq_product <- paste0("sel_prod_scores_",i,"$product=gsub('_growth','',top_products[",i,"])")
  eval(parse(text=eq_product))

  if (i==1) {
    # sel_prod_scores_1$pred.product.norm = sel_prod_scores_1$pred.product
    # sel_prod_scores_1$pred.cal = sel_prod_scores_1$pred.product
    all_scores=data.frame()
        }
  all_scores=dplyr::bind_rows(all_scores,get(paste0("sel_prod_scores_",i)))
}
...
# 
# save(actual_current_products_active,file="C:/Santander/data/actual_current_products_active.RData")
...
write.csv(submission[,1:2],file="C:/Santander/Submissions/submission33_XGboost_6lags_plus_v2_combined_models_isotonic_calib_Dec03.csv",row.names = FALSE)
