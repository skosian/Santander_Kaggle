...
splits <- h2o.splitFrame(data = mod.hex, ratios = c(0.2,0.2), destination_frames = c("train.hex", "valid.hex","extra.hex"), seed = 1234)
...
grid_id = paste0("grid_5perc_",i)

hyper_params = list( 
                      max_depth = c(5,6,7,8,10),
                      sample_rate= c(0.1,0.2,0.4,0.6,0.8),
                      col_sample_rate= c(0.1,0.2,0.4,0.6),
                      min_rows=c(25,50,100),
                      nbins=c(20,50,100),
                      nbins_cats = c(10,20,50),
                      histogram_type = c("UniformAdaptive","QuantilesGlobal","RoundRobin"),
                      sample_rate_per_class= list(c(0.05,1)) #list(c(0.5,1),c(1,1),c(0.2,1))
                     )
...
# 
assign(paste0("scores_5perc",i),cbind(test_raw_ids,as.data.frame(get(paste0("fit.scoring_",i)))))
...
# Get the mean valus of predictions on all data sets
 ...
 h2o.saveModel(get(paste0("best_model_from_grid",i)),paste0("C:/Santander/Saved Models/best_6lag_plus_GBM_5perc_Nov15/model",i))
}

# best_model <-h2o.getModel(sortedGrid@model_ids[[1]])


auc_report_6lags_plus_GBM_5perc_Nov15 <- auc_report
mean_predictions_6lags_plus_GBM_5perc_Nov15 <- mean_predictions

write.csv(auc_report_6lags_plus_GBM_5perc_Nov15,file="C:/Santander/Stats/auc_report_6lags_plus_GBM_5perc_Nov15.csv")
write.csv(mean_predictions_6lags_plus_GBM_5perc_Nov15,file="C:/Santander/Stats/mean_predictions_6lags_plus_GBM_5perc_Nov15.csv")
...
{ 
  eq1 <- paste0("scores_5perc",j,"$p0=NULL")
  eval(parse(text=eq1))
  eq2 <- paste0("scores_5perc",j,"$prediction=scores_5perc",j,"$p1*prediction_modifier[",j,"]" )
  eval(parse(text=eq2))
  eq3 <- paste0("scores_5perc",j,"$product=gsub('_growth','',top_products[",j,"])")
  eval(parse(text=eq3))
  
  if (j==1)
  {
    all_scores=get(paste0("scores_5perc",j))
  } else {
    all_scores=rbind(all_scores,get(paste0("scores_5perc",j)))
    }
}
...
save(all_scores_combined_string,file="C:/Santander/Data/all_scores_combined_gbm_6lags_plus_5perc_Nov15.RData")


submission=all_scores_combined_string[,c("ncodpers","output_string")]
names(submission)=c("ncodpers","added_products")

write.csv(submission[,1:2],file="C:/Santander/Submissions/submission13_Nov15_gbm_6lags_plus_5perc.csv",row.names = FALSE)
