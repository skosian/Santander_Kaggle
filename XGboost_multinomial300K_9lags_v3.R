...
train300K_9lags_v3$tipodom = train300K_9lags_v3$nomprov = train300K_9lags_v3$ult_fec_cli_1t = train300K_9lags_v3$fecha_alta = NULL

####################################################################################################################################
### MISSING VALUE IMPUTATION: Impute missing values with medians ###################################################################
####################################################################################################################################
all_columns <- names(train300K_9lags_v3)
stats.train300K_9lags_v3 <- stat.desc(train300K_9lags_v3[,all_columns])

write.csv(stats.train300K_9lags_v3,file="C:/Santander/EDA/stats.train300K_9lags_v3_Dec08.csv")
# load("C:/Santander/EDA/stats.train300K_9lags_v3_Nov30.csv")

numeric_columns_train300K <- names(train300K_9lags_v3[,sapply(train300K_9lags_v3,is.numeric)])
...
names(train300K_9lags_v3)[order(names(train300K_9lags_v3))]


train300K_9lags_v3_imputed <- train300K_9lags_v3

# imputation with median
for (i in 1:length(numeric_columns_train300K)) {
  train300K_9lags_v3_imputed[,numeric_columns_train300K[i]][is.na(train300K_9lags_v3_imputed[,numeric_columns_train300K[i]])] <- stats.train300K_9lags_v3[8,numeric_columns_train300K[i]]  
}

test_9lags_v3_imputed <- test_9lags_v3

for (i in 1:length(numeric_columns_test)) {
  test_9lags_v3_imputed[,numeric_columns_test[i]][is.na(test_9lags_v3_imputed[,numeric_columns_test[i]])] <- stats.train300K_9lags_v3[8,numeric_columns_test[i]]  
}

save(test_9lags_v3_imputed ,file="C:/Santander/Data/test_9lags_v3_imputed.RData")
save(train300K_9lags_v3_imputed ,file="C:/Santander/Data/train300K_9lags_v3_imputed.RData")

load("C:/Santander/Data/test_9lags_v3_imputed.RData")
load("C:/Santander/Data/train300K_9lags_v3_imputed.RData")

##############################################################################################################################################

# Select only non-zero growth records, and create a column called new_product that identifies the new product name
for (i in 1:12) {
   assign(paste0("train_growth",i),subset(train300K_9lags_v3_imputed,get(top_products[i])==1))
...
scoring_short <- test_9lags_v3_imputed[,x]
...
write.csv(submission[,1:2],file="C:/Santander/Submissions/submission40_Multinomial_XGboost_6lags_v3_eta0.1_Dec08_logloss.csv",row.names = FALSE)
...
# train300K_with_lags_numeric=train300K_with_lags
# for (i in 1:length(top_products)) {
#   train300K_with_lags_numeric[,top_products[i]] = as.numeric(as.character(train300K_with_lags[,top_products[i]]))
# }
# 
# mod_numeric=dplyr::filter(train300K_with_lags_numeric,fecha_dato >'2015-03-28' & fecha_dato <= '2016-03-28')
# val_numeric=dplyr::filter(train300K_with_lags_numeric,fecha_dato > '2016-03-28')
# 
