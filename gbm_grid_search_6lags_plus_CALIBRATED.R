...
splits <- h2o.splitFrame(data = mod.hex, ratios = c(0.4,0.2), destination_frames = c("train.hex", "valid.hex","extra.hex"), seed = 1234)
...
train_hex = as.data.frame(train.hex)
train_hex_ids=data.frame(ncodpers=train_hex$ncodpers,fecha_dato=train_hex$fecha_dato)

valid_hex = as.data.frame(valid.hex)
valid_hex_ids=data.frame(ncodpers=valid_hex$ncodpers,fecha_dato=valid_hex$fecha_dato)

test_hex = as.data.frame(test.hex)
test_hex_ids=data.frame(ncodpers=test_hex$ncodpers,fecha_dato=test_hex$fecha_dato)

# train_sample=as.data.frame(train.hex)
# 
# zeros = subset(train_sample,num_new_products_factor == "0")
# ones = subset(train_sample,num_new_products_factor != "0")
# zeros_sample=dplyr::sample_frac(zeros,size=0.5)
# 
# train_stratified.hex <- as.h2o(as.data.frame(rbind(ones,zeros_sample)))

for (i in 2:9) #length(top_products)) 
  {

y <- top_products[i]

train_hex_y <- as.numeric(as.character(train_hex[,y]))
valid_hex_y <- as.numeric(as.character(valid_hex[,y]))
test_hex_y <- as.numeric(as.character(test_hex[,y]))

grid_id = paste0("grid_20pct_",i)

# hyper_params = list( 
#                       max_depth = 8, #c(5,6,7,8,10,12),
#                       sample_rate=0.6, #c(0.1,0.2,0.4,0.6,0.8),
#                       col_sample_rate=0.1, #c(0.1,0.2,0.4,0.6),
#                        min_rows=100, #c(25,50,100),
#                        nbins=100, #c(20,50,100),
#                        nbins_cats = 10, #c(10,20,50),
#                       # histogram_type = c("UniformAdaptive","QuantilesGlobal","RoundRobin"),
#                       sample_rate_per_class= list(c(0.5,1)) #list(c(0.5,1),c(1,1),c(0.2,1))
#                      )
...
# assign(paste0("modid",i),list.files(path=paste0("C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/model",i)))
# assign(paste0("best_model_from_grid",i),h2o.loadModel(path=paste0("C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/model",i,"/",get(paste0("modid",i)))))
assign(paste0("best_model_from_grid",i),h2o.loadModel(path=paste0("C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/model",i,"/bestmodel")))
...
assign(paste0("train_scores_20pct_",i),cbind(train_hex_ids,train_hex_y,as.data.frame(get(paste0("fit.train_",i)))))
assign(paste0("valid_scores_20pct_",i),cbind(valid_hex_ids,valid_hex_y,as.data.frame(get(paste0("fit.valid_",i)))))
assign(paste0("test_scores_20pct_",i),cbind(test_hex_ids,test_hex_y,as.data.frame(get(paste0("fit.test_",i))))) 
assign(paste0("scores_20pct_",i),cbind(test_raw_ids,as.data.frame(get(paste0("fit.scoring_",i)))))

eq <-  paste0("save(train_scores_20pct_",i,", valid_scores_20pct_",i,", test_scores_20pct_",i,", scores_20pct_",i,",file='C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/Scores/model_20pct_",i,"_scores.RData')")
eval(parse(text=eq))

## Get calibration reports

#filter out missing values from target columns
assign(paste0("valid_scores_20pct_",i),dplyr::filter(get(paste0("valid_scores_20pct_",i)),!is.na(valid_hex_y)))
assign(paste0("test_scores_20pct_",i),dplyr::filter(get(paste0("test_scores_20pct_",i)),!is.na(test_hex_y)))

Calibration(
  FileType = "rdf",
  RDataFrameTrain = get(paste0("valid_scores_20pct_",i)),
  ActualColumnNameTrain = "valid_hex_y",
  PredictedColumnNameTrain = "p1",
  RDataFrameTest = get(paste0("test_scores_20pct_",i)),
  ActualColumnNameTest = "test_hex_y",
  PredictedColumnNameTest = "p1", 
  PredictedVariableName ="Product Propensity",
  Target = "binary",
  Description = paste0("Model_20pct_",i,"_Valid_Test"),
  outputDir = "C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/Calibration/")
...
#  h2o.saveModel(get(paste0("best_model_from_grid",i)),paste0("C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/model",i),force = T)
}

# best_model <-h2o.getModel(sortedGrid@model_ids[[1]])


auc_report_6lags_plus_GBM_20pct_Nov16 <- auc_report
mean_predictions_6lags_plus_GBM_20pct_Nov16 <- mean_predictions

write.csv(auc_report_6lags_plus_GBM_20pct_Nov16,file="C:/Santander/Stats/auc_report_6lags_plus_GBM_20pct_Nov16.csv")
write.csv(mean_predictions_6lags_plus_GBM_20pct_Nov16,file="C:/Santander/Stats/mean_predictions_6lags_plus_GBM_20pct_Nov16.csv")
...
{ 
  eq1 <- paste0("scores_20pct_",j,"$p0=NULL")
  eval(parse(text=eq1))
  eq2 <- paste0("scores_20pct_adj_",j,"$prediction=scores_20pct_",j,"$p1*prediction_modifier[",j,"]" )
  ...
  eval(parse(text=eq2))
  eq3 <- paste0("scores_20pct_adj_",j,"$product=gsub('_growth','',top_products[",j,"])")
  eval(parse(text=eq3))
  
  if (j==1)
  {
    all_scores=get(paste0("scores_20pct_adj_",j))
  } else {
    all_scores=rbind(all_scores,get(paste0("scores_20pct_adj_",j)))
    }
}


all_scores_sorted  <- dplyr::arrange(all_scores,ncodpers,desc(prediction))
all_scores_sorted$p1=all_scores_sorted$predict= NULL

#######################################################################################################################
### Perform calibration by Isotonic method                                                               ##############
#######################################################################################################################

for (i in 1:9) {

  load(paste0("C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/Scores/model_20pct_",i,"_scores.RData"))
  
  eq_product <- paste0("scores_20pct_",i,"$product=gsub('_growth','',top_products[",i,"])")
  eval(parse(text=eq_product))

    ## Isotonic calibration models
    print(paste("Processing Isotonic for i=",i))
    
    load(paste0("C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/Calibration/iso.calib.model.rda.Model_20pct_",i,"_Valid_Test"))
    
    eq_iso<- paste0("train_scores_20pct_",i," <- cbind(train_scores_20pct_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(train_scores_20pct_",i,"$p1)))")
    eval(parse(text=eq_iso))
    eq_iso<- paste0("valid_scores_20pct_",i," <- cbind(valid_scores_20pct_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(valid_scores_20pct_",i,"$p1)))")
    eval(parse(text=eq_iso))
    eq_iso<- paste0("test_scores_20pct_",i," <- cbind(test_scores_20pct_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(test_scores_20pct_",i,"$p1)))")
    eval(parse(text=eq_iso))
    eq_iso<- paste0("scores_20pct_",i," <- cbind(scores_20pct_",i,",data.frame(pred.cal = stats::as.stepfun(iRFit)(scores_20pct_",i,"$p1)))")
    eval(parse(text=eq_iso))
    
   eq <-  paste0("save(train_scores_20pct_",i,", valid_scores_20pct_",i,", test_scores_20pct_",i,", scores_20pct_",i,",file='C:/Santander/Saved Models/best_6lag_plus_GBM_20pct_Nov16/Scores/Calibrated Scores/model_",i,"_scores_20pct_isotonic.RData')")
  eval(parse(text=eq))
  if (i==1)
  {
    all_scores_20pct=get(paste0("scores_20pct_",i))
  } else {
    all_scores_20pct=rbind(all_scores_20pct,get(paste0("scores_20pct_",i)))
  }    
}

all_scores_sorted  <- dplyr::arrange(all_scores_20pct,ncodpers,desc(pred.cal))
...
all_scores_sorted2$prod_num <- as.integer(rownames(all_scores_sorted2))  %% 9
all_scores_sorted2$prod_num[all_scores_sorted2$prod_num == 0] <- 9
...
pred_num_products_scoring = h2o.predict(best_gbm_num_products,newdata = scoring.hex)
 ...
 pred_num_products_scoring1 <- pred_num_products_scoring[,1:2]
...
save(all_scores_combined_string,file="C:/Santander/Data/all_scores_combined_gbm_6lags_plus_Nov16_20pct_isotonic.RData")


submission=all_scores_combined_string[,c("ncodpers","output_string")]
names(submission)=c("ncodpers","added_products")

write.csv(submission[,1:2],file="C:/Santander/Submissions/submission21_Nov16_gbm_6lags_plus_20pct_isotonic.csv",row.names = FALSE)
